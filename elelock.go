...
type Elelock struct {
	clk, reset, close, tenkey, lock, match, SECRET_0, SECRET_1, SECRET_2, SECRET_3 *variable.BitArray
	key                                                                            []*variable.BitArray
}

func NewElelock() Elelock {
	args := &Elelock{variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(10), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), make([]*variable.BitArray, 4)}
...
func NewGoroutineElelock(in []chan int, out []chan int) *Elelock {
	elelock := &Elelock{variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(10), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), variable.NewBitArray(1), make([]*variable.BitArray, 4)}
...
func (Elelock *Elelock) PreAlways1() []variable.BitArray {
	...
	if variable.CheckBit(variable.CreateBits("1'b1").Equal(*Elelock.reset).Or(variable.CreateBits("1'b1").Equal(*Elelock.close))) {
		var6.Assign(*variable.CreateBits("4'b1111"))
		var7.Assign(*variable.CreateBits("4'b1111"))
		var8.Assign(*variable.CreateBits("4'b1111"))
		var9.Assign(*variable.CreateBits("4'b1111"))
	} else {
		var10.Assign(*Elelock.key[2])
		var11.Assign(*Elelock.key[1])
		var12.Assign(*Elelock.key[0])
		var13.Assign(Elelock.keyenc(*Elelock.tenkey))
	}
	return []variable.BitArray{var1, var2, var3, var4, var5, var6, var7, var8, var9, var10, var11, var12, var13}
}
...
func (Elelock *Elelock) PreAlways2() []variable.BitArray {
	...
	if variable.CheckBit(*Elelock.match) {
		var6.Assign(*variable.CreateBits("1'b0"))
	} else {
		if variable.CheckBit(variable.CreateBits("1'b1").Equal(*Elelock.reset).Or(variable.CreateBits("1'b1").Equal(*Elelock.close))) {
...
func (Elelock *Elelock) Always1(vars []variable.BitArray) {
	if variable.CheckBit(variable.CreateBits("1'b1").Equal(*Elelock.reset).Or(variable.CreateBits("1'b1").Equal(*Elelock.close))) {
		Elelock.key[3].Assign(vars[5])
		Elelock.key[2].Assign(vars[6])
		Elelock.key[1].Assign(vars[7])
		Elelock.key[0].Assign(vars[8])
	} else {
		Elelock.key[3].Assign(vars[9])
		Elelock.key[2].Assign(vars[10])
		Elelock.key[1].Assign(vars[11])
		Elelock.key[0].Assign(vars[12])
	}
}
